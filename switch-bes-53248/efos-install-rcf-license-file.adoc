...
summary: 'Dopo aver completato la configurazione iniziale, installa il file di configurazione di riferimento (RCF) e il file di licenza sullo switch cluster Broadcom BES-53248.' 
---
= Installa il file RCF e il file di licenza sullo switch di cluster Broadcom BES-53248
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Installa il file di configurazione di riferimento (RCF) e il file di licenza insieme sullo switch di cluster Broadcom BES-53248. A partire da EFOS 3.12.0.1, puoi installare entrambi i file dopo aver completato la configurazione iniziale dello switch.


CAUTION: Tutte le porte sono configurate quando si installa RCF, ma è necessario installare la licenza per attivare le porte configurate.
...
* Il RCF attuale, disponibile dalla pagina link:https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab["Switch cluster Broadcom"^].
...
. Copiare l'RCF nel bootflash dello switch cs2 utilizzando uno dei seguenti protocolli di trasferimento: FTP, HTTP, TFTP, SFTP o SCP.
+
Questo esempio mostra l'utilizzo di HTTP per copiare un RCF nel bootflash sullo switch cs2:
...
----
====
. Sullo switch, verifica che le modifiche siano state apportate:
+
[source, cli]
----
show running-config
...
--
====
. [[step21]]Sullo switch del cluster cs1, chiudere le porte connesse alle porte del cluster dei nodi.
+
...
.Cosa succederà ora?
Dopo aver installato il file RCF e la licenza, puoi link:configure-ssh.html["abilita SSH"].
