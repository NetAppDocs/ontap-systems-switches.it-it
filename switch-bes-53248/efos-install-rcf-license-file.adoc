...
[role="lead"]
A partire da EFOS 3.12.0.1, è possibile installare il file di configurazione di riferimento (RCF) e il file di licenza dopo aver configurato lo switch del cluster BES-53248.


CAUTION: Tutte le porte vengono configurate durante l'installazione di RCF, ma è necessario installare la licenza per attivare le porte configurate.
...
* RCF corrente, disponibile nella https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab["Broadcom Cluster Switch"^] pagina.
...
. Copiare l'RCF nella memoria di avvio dello switch CS2 utilizzando uno dei seguenti protocolli di trasferimento: FTP, HTTP, TFTP, SFTP o SCP.
+
In questo esempio viene illustrato l'utilizzo di HTTP per copiare un RCF nel bootflash dello switch CS2:
...
----
====
. Sullo switch, verificare che le modifiche siano state apportate:
+
[source, cli]
----
show running-config
...
--
====
. [[step21]]sullo switch del cluster CS1, chiudere le porte collegate alle porte del cluster dei nodi.
+
