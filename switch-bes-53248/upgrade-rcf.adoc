...
summary: 'Aggiorna il file di configurazione di riferimento (RCF) sullo switch cluster Broadcom BES-53248 dopo aver aggiornato il software EFOS.' 
---
= Aggiorna l'RCF sullo switch di cluster Broadcom BES-53248
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Aggiorna il file di configurazione (RCF) sullo switch di cluster Broadcom BES-53248 dopo aver aggiornato il software EFOS e applicato le eventuali licenze necessarie.
...
* È necessaria una connessione console allo switch, richiesta durante l'installazione dell'RCF partendo dalle impostazioni di fabbrica.
...
Utilizzi la CLI dello switch e la CLI di ONTAP (come indicato nei passaggi successivi) per la configurazione delle porte. Configuri le porte sul primo switch (cs2) e poi ripeti questi passaggi per il secondo switch (cs1).
...
CAUTION: Quando aggiorni o applichi un nuovo RCF, devi cancellare le impostazioni dello switch ed eseguire la configurazione di base. Devi essere connesso allo switch tramite la console seriale per cancellare le impostazioni dello switch.
...
--
====
. [[step20]]Ripeti i passaggi da 1 a 19 sullo switch cs1.
...
--
====
. [[step6]] Cambia di nuovo il livello di privilegio su admin:
+
[source, cli]
----
set -privilege admin
...
.Cosa succederà ora?
Dopo aver aggiornato il tuo RCF, puoi link:configure-ssh.html["Abilita SSH per la raccolta dei log"].
