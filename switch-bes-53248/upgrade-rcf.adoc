...
summary: 'È possibile aggiornare l"RCF dopo aver configurato lo switch di rete BES-53248 e dopo aver applicato eventuali nuove chiavi di licenza.' 
---
= Aggiorna la versione RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
È possibile aggiornare l'RCF dopo aver aggiornato EFOS dello switch di cluster BES-53248 e dopo aver applicato eventuali nuove licenze.
...
* Fare riferimento al software e alle guide di aggiornamento appropriate disponibili sul link:https://www.broadcom.com/support/bes-switch["Broadcom"^] sito per la documentazione completa sulle procedure di aggiornamento e downgrade dello switch BES-53248.
...
* Gli esempi in questa procedura utilizzano quattro nodi. Questi nodi utilizzano due porte di interconnessione cluster 10GbE `e0a` e `e0b`. Fare riferimento a link:https://hwu.netapp.com/Home/Index["Hardware Universe"^] per verificare le porte di interconnessione cluster corrette sulle proprie piattaforme.
...
show isdp neighbor
----
. Sullo switch cs2, chiudere le porte collegate alle porte del cluster dei nodi.  Ad esempio, se le porte da 0/1 a 0/16 sono collegate ai nodi ONTAP :
+
[listing, subs="+quotes"]
----
(cs2)> *enable*
...
. Notare quanto segue:
+
.. Se sullo switch sono state installate licenze per porte aggiuntive, è necessario modificare il file RCF per configurare le porte con licenza aggiuntive. Fare riferimento a link:configure-licenses.html#activate-newly-licensed-ports["Attiva le porte appena concesse in licenza"] per i dettagli. Tuttavia, quando si esegue l'aggiornamento a RCF 1.12 o versioni successive, le modifiche non sono più necessarie poiché tutte le interfacce sono ora preconfigurate.
...
CAUTION: Il file deve essere denominato  `reference-config.scr` quando lo si carica e si applica il file RCF, altrimenti la fase di applicazione dello script non andrà a buon fine.

. Copia il file RCF sullo switch cs2 utilizzando uno dei seguenti protocolli di trasferimento: HTTP, HTTPS, FTP, SFTP o SCP.
+
...
--
====
. [[step11]]Esaminare l'output del banner dal `show clibanner` comando. Per garantire la corretta configurazione e il corretto funzionamento dello switch, è necessario leggere e seguire queste istruzioni.
+
`show clibanner`
...
----
. Sullo switch del cluster cs2, richiamare le porte connesse alle porte del cluster dei nodi.
+
[listing, subs="+quotes"]
----
(cs2)> *enable*
...
--
====
. [[step21]]Ripetere i passaggi da 1 a 20 sullo switch cs1.
