...
summary: 'Installa il file di configurazione di riferimento (RCF) sullo switch cluster Broadcom BES-53248 dopo aver completato la configurazione iniziale e aver applicato le eventuali licenze necessarie.' 
---
= Installa l'RCF sullo switch di cluster Broadcom BES-53248
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Installa il file di configurazione di riferimento (RCF) sullo switch cluster Broadcom BES-53248 dopo aver completato la configurazione iniziale e aver applicato le eventuali licenze necessarie. L'RCF definisce le assegnazioni delle porte, le VLAN e le impostazioni di rete per il funzionamento del cluster.


CAUTION: Per EFOS 3.12 e versioni successive, seguire i passaggi di installazione in link:efos-install-rcf-license-file.html["Installare il file di configurazione (RCF) e il file di licenza"].
...
* Il file RCF corrente, disponibile dalla pagina link:https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab["Switch cluster Broadcom"^].
...
* È necessaria una connessione console allo switch, richiesta durante l'installazione dell'RCF partendo dalle impostazioni di fabbrica. Questo requisito è facoltativo se si è utilizzato l'articolo della Knowledge Base link:https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["Come cancellare la configurazione su uno switch di interconnessione Broadcom mantenendo la connettività remota"^] per cancellare la configurazione in precedenza.


.Documentazione suggerita
Consultare la tabella di compatibilità dello switch per le versioni di ONTAP e RCF supportate. Fare riferimento a link:https://mysupport.netapp.com/site/info/broadcom-cluster-switch["Scarica il software EFOS"^]. Si noti che potrebbero esserci dipendenze tra la sintassi dei comandi in RCF e quella presente nelle versioni di EFOS.
...
* Gli esempi in questa procedura utilizzano quattro nodi. Questi nodi utilizzano due porte di interconnessione cluster 10GbE  `e0a` e  `e0b`. Fare riferimento alla link:https://hwu.netapp.com/Home/Index["Hardware Universe"^] per verificare le porte cluster corrette sulle proprie piattaforme.
...
. Notare quanto segue:
+
.. Se sullo switch sono state installate licenze per porte aggiuntive, è necessario modificare il file RCF per configurare le porte con licenza aggiuntive. Consultare link:configure-licenses.html#activate-newly-licensed-ports["Attiva le porte appena concesse in licenza"] per i dettagli.
