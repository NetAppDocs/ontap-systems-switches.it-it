...
summary: Migra un cluster senza switch a due nodi in un ambiente cluster con switch utilizzando gli switch di cluster Broadcom BES-53248 per consentire la scalabilità oltre i due nodi. 
---
= Migra a un cluster a due nodi con switch di cluster Broadcom BES-53248
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Esegui la migrazione di un cluster senza switch a un ambiente cluster con switch utilizzando gli switch cluster Broadcom BES-53248. Questa migrazione senza interruzioni ti permette di scalare il cluster oltre due nodi.
...
+
Il link:https://hwu.netapp.com/Home/Index["Hardware Universe"^] contiene le informazioni più recenti sulle porte del cluster effettive per le tue piattaforme.
...
+
Il link:https://hwu.netapp.com/Home/Index["Hardware Universe"^] contiene maggiori informazioni sul cablaggio.
