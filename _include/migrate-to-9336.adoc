...
. Verificare le connessioni delle porte dello switch dal punto di vista dello switch cs2-new:
+
[listing, subs="+quotes"]
----
...
----
. Sullo switch del cluster cs1, spegnere le porte collegate alle porte del cluster dei nodi. Nell'esempio seguente viene utilizzato l'output dell'esempio di interfaccia del passo 7.
+
[listing, subs="+quotes"]
----
...
----
+
Tutte le LIF del cluster passano al nuovo switch cs2.

. Verificare che le LIF del cluster siano migrate alle porte ospitate sullo switch cs2-new. Questa operazione potrebbe richiedere alcuni secondi:
+
`network interface show -vserver Cluster`
...
. Verificare le connessioni delle porte dello switch dal punto di vista dello switch cs1-new:
+
.Mostra esempio
[%collapsible]
====
...
----
====
. Abilitare la funzione di raccolta dei log dello switch Ethernet per la raccolta dei file di log relativi allo switch.
