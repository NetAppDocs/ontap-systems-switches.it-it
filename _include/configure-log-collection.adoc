...
* Per la raccolta dei log con gli switch Broadcom e Cisco, è necessario creare un nuovo utente sullo switch per ogni configurazione del cluster con la raccolta dei log attivata. Questi switch non supportano più chiavi SSH per lo stesso utente. Qualsiasi impostazione di raccolta di log aggiuntiva eseguita sovrascrive qualsiasi chiave SSH preesistente per l'utente.
