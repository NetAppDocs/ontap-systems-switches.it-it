...
summary: È possibile migrare i vecchi switch cluster Cisco non disruptivi per un cluster ONTAP verso gli switch di rete cluster Cisco Nexus 92300YC. 
---
...
[role="lead"]
È possibile migrare i vecchi switch cluster Cisco non disruptivi per un cluster ONTAP verso gli switch di rete cluster Cisco Nexus 92300YC.
...
* Un cluster esistente completamente funzionante.
* Connettività 10 GbE e 40 GbE dai nodi agli switch cluster Nexus 92300YC.
* Tutte le porte del cluster sono attive per garantire operazioni senza interruzioni.
* Versione corretta di NX-OS e file di configurazione di riferimento (RCF) installati sugli switch del cluster Nexus 92300YC.
...
* I nuovi switch cluster Nexus 92300YC sono cs1 e cs2.
...
* Lo switch c2 viene sostituito prima dallo switch cs2 e poi lo switch c1 viene sostituito dallo switch cs1.
+
** Un ISL temporaneo è costruito su cs1 collegando c1 a cs1.
...
* Alcune porte sono configurate sugli switch Nexus 92300YC per funzionare a 10 GbE o 40 GbE.
...
* https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe - Interruttori"^]contiene informazioni sul cablaggio supportato per tutti gli switch cluster.
* Le versioni ONTAP e NX-OS supportate in questa procedura sono disponibili su https://support.netapp.com/NOW/download/software/cm_switches/["Switch Ethernet Cisco"^] pagina.
...
. Verificare che le porte del cluster su ciascun nodo siano connesse agli switch del cluster esistenti nel modo seguente (dal punto di vista dei nodi) utilizzando il comando:
+
...
----
====
. Verificare che le porte e gli switch del cluster siano connessi nel modo seguente (dal punto di vista degli switch) utilizzando il comando:
+
`show cdp neighbors`
...
. Configurare un ISL temporaneo su cs1 sulle porte e1/41-48, tra c1 e cs1.
+
.Mostra esempio
[%collapsible]
====
L'esempio seguente mostra come il nuovo ISL è configurato su c1 e cs1:
...
----
====
. Rimuovere i cavi ISL dalle porte e1/41-48 da c2 e collegare i cavi alle porte e1/41-48 su cs1.
. Verificare che le porte ISL e il port-channel siano operativi e colleghino c1 e cs1:
+
...
[%collapsible]
====
L'esempio seguente mostra il comando Cisco show port-channel summary utilizzato per verificare che le porte ISL siano operative su c1 e cs1:
...
----
====
. Per node1, scollegare il cavo da e1/1 su c2, quindi collegare il cavo a e1/1 su cs2, utilizzando il cablaggio appropriato supportato da Nexus 92300YC.
. Per node2, scollegare il cavo da e1/2 su c2, quindi collegare il cavo a e1/2 su cs2, utilizzando il cablaggio appropriato supportato da Nexus 92300YC.
...
----
====
. Per node1, scollegare il cavo da e1/1 su c1, quindi collegare il cavo a e1/1 su cs1, utilizzando il cablaggio appropriato supportato da Nexus 92300YC.
. Per node2, scollegare il cavo da e1/2 su c1, quindi collegare il cavo a e1/2 su cs1, utilizzando il cablaggio appropriato supportato da Nexus 92300YC.
. Le porte del cluster su ciascun nodo sono ora collegate agli switch del cluster nel modo seguente, dal punto di vista dei nodi:
+
`network device-discovery show -protocol cdp`
...
=== Passaggio 3: Completare la migrazione
...
--
====
. [[step3]] Se hai disattivato la creazione automatica dei casi, riattivala richiamando un messaggio AutoSupport :
+
