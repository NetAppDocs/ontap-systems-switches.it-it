...
[role="lead"]
Seguire questa procedura per installare RCF dopo aver configurato lo switch Nexus 92300YC per la prima volta. È inoltre possibile utilizzare questa procedura per aggiornare la versione di RCF.
...
----
====
.. Verificare che tutte le interfacce del cluster (LIF) si trovino sulla porta home: +
...
--
Per ONTAP 9.8 e versioni successive, attivare la funzione di raccolta dei log del monitor dello stato dello switch del cluster per la raccolta dei file di log relativi allo switch, utilizzando i comandi seguenti:
...
link:setup-install-cshm-file.html["Installare il file di configurazione di Cluster Switch Health Monitor (CSHM)"].
