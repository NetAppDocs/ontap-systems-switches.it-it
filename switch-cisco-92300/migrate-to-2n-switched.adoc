...
summary: Se hai un ambiente cluster senza switch a due nodi già esistente, puoi migrare a un ambiente cluster con switch a due nodi utilizzando gli switch Cisco Nexus 92300YC per permetterti di scalare oltre i due nodi nel cluster. 
---
= Esegui la migrazione a un cluster a due nodi con uno switch Cisco Nexus 92300YC
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Se hai un ambiente cluster senza switch esistente a due nodi, puoi migrare a un ambiente cluster con switch a due nodi utilizzando gli switch Cisco Nexus 92300YC per permetterti di scalare oltre i due nodi nel cluster.

La procedura da seguire dipende dal fatto che su ciascun controller siano presenti due porte dedicate per la rete cluster o una singola porta cluster. La procedura documentata funziona per tutti i nodi che utilizzano porte ottiche o twinax, ma non è supportata su questo switch se i nodi utilizzano porte RJ45 10Gb BASE-T integrate per le porte di rete cluster.

La maggior parte dei sistemi richiede due porte di rete cluster dedicate su ciascun controller.


NOTE: Al termine della migrazione, potrebbe essere necessario installare il file di configurazione richiesto per supportare il Cluster Switch Health Monitor (CSHM) per gli switch cluster 92300YC. Consulta link:../switch-cshm/cshm-overview.html["Monitoraggio dello stato di salute degli switch (CSHM)"].
...
* Le connessioni nodo-nodo e switch-to-switch dello switch Nexus 92300YC utilizzano cavi twinax o fibra.
+
Il link:https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe - Interruttori"^] contiene maggiori informazioni sul cablaggio.

* I cavi Inter-Switch Link (ISL) sono collegati alle porte 1/65 e 1/66 su entrambi gli switch 92300YC.
* La personalizzazione iniziale di entrambi gli switch 92300YC è stata completata. Quindi:
+
** Gli switch 92300YC eseguono la versione più recente del software
...
+
Il link:https://hwu.netapp.com["Hardware Universe"^] contiene le informazioni più recenti sulle porte del cluster effettive per le tue piattaforme.
...
[%collapsible]
====
Il seguente esempio mostra che le porte rivolte verso il nodo da 1 a 64 sono disabilitate sullo switch cs1:
...
----
====
. Verifica che l'ISL e le porte fisiche sull'ISL tra i due switch 92300YC cs1 e cs2 siano attive sulle porte 1/65 e 1/66:
+
...
----
+ Il seguente esempio elenca i dispositivi vicini sullo switch cs2:
...
----
====
. Scollega il cavo dalla porta cluster e0a su node1, poi collega e0a alla porta 1 sul cluster switch cs1, usando il cablaggio appropriato supportato dagli switch 92300YC.
+
Il link:https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe - Interruttori"^] contiene maggiori informazioni sul cablaggio.

. Scollega il cavo dalla porta cluster e0a su node2, poi collega e0a alla porta 2 sullo switch cluster cs1, usando il cablaggio appropriato supportato dagli switch 92300YC.
. Abilitare tutte le porte rivolte verso il nodo sullo switch del cluster cs1.
+
.Mostra esempio
[%collapsible]
====
Il seguente esempio mostra che le porte da 1/1 a 1/64 sono abilitate sullo switch cs1:
...
----
====
. Verifica che tutti i LIF del cluster siano attivi, operativi e visualizzati come true per  `Is Home`:
+
`network interface show -vserver Cluster`
...
----
====
. Scollega il cavo dalla porta cluster e0b su node1, poi collega e0b alla porta 1 sul cluster switch cs2, usando il cablaggio appropriato supportato dagli switch 92300YC.
. Scollega il cavo dalla porta cluster e0b su node2, quindi collega e0b alla porta 2 sullo switch cluster cs2, usando il cablaggio appropriato supportato dagli switch 92300YC.
...
--
====
. [[step8]] Se hai disabilitato la creazione automatica dei casi, riattivala richiamando un messaggio AutoSupport:
+
