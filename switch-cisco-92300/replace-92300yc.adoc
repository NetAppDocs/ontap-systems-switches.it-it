...
summary: Sostituisci uno switch di cluster Cisco Nexus 92300YC difettoso in una rete di cluster. Questa è una procedura senza interruzioni. 
---
= Sostituisci uno switch di rete cluster Cisco Nexus 92300YC
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sostituisci uno switch di cluster Cisco Nexus 92300YC difettoso in una rete di cluster senza interrompere il funzionamento del cluster. Questa procedura non distruttiva (NDU) prevede la preparazione di uno switch sostitutivo e la migrazione delle connessioni dallo switch guasto.
...
.Prima di iniziare
Prima di procedere alla sostituzione dello switch, assicurati che:
...
* Per lo switch di ricambio Nexus 92300YC:
+
...
** Le connessioni del nodo sono le porte da 1/1 a 1/64.
...
** La personalizzazione iniziale dello switch è completa, come descritto in dettaglio in: link:configure-install-initial.html["Configura lo switch Cisco Nexus 92300YC"].
+
...
* Gli switch Nexus 92300YC attualmente in uso si chiamano cs1 e cs2.
...
.. Fai clic sulla freccia Indietro del tuo browser per tornare alla pagina *Descrizione*, fai clic su *CONTINUA*, accetta il contratto di licenza e poi vai alla pagina *Download*.
...
. Spegni le porte ISL 1/65 e 1/66 sullo switch Nexus 92300YC cs1:
+
.Mostra esempio
[%collapsible]
====
...
----
====
. Scollega tutti i cavi dallo switch Nexus 92300YC cs2 e collegali alle stesse porte dello switch Nexus 92300YC newcs2.
. Attiva le porte ISL 1/65 e 1/66 tra gli switch cs1 e newcs2, poi verifica lo stato di funzionamento del port channel.
+
Port-Channel dovrebbe indicare Po1(SU) e Member Ports dovrebbe indicare Eth1/65(P) e Eth1/66(P).

+
.Mostra esempio
[%collapsible]
====
Questo esempio abilita le porte ISL 1/65 e 1/66 e visualizza il riepilogo del port channel sullo switch cs1:
...
--
====
. [[step2]] Conferma la seguente configurazione di rete del cluster:
+
`network port show`
