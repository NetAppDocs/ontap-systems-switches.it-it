...
summary: È possibile installare lo switch di archiviazione Cisco Nexus 3232C e il pannello pass-through in un NetApp cabinet con le staffe standard incluse con lo switch. 
---
= Installare uno switch di archiviazione Cisco Nexus 3232C in un NetApp cabinet
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
A seconda della configurazione, potrebbe essere necessario installare lo switch di archiviazione Cisco Nexus 3232C e il pannello pass-through in un NetApp cabinet con le staffe standard incluse con lo switch.

.Prima di iniziare
Verificare di disporre di quanto segue: * I requisiti di preparazione iniziale, il contenuto del kit e le precauzioni di sicurezza nel link:http://www.cisco.com/c/en/us/td/docs/switches/datacenter/nexus3000/hw/installation/guide/b_n3000_hardware_install_guide.html["Guida all'installazione hardware Cisco Nexus 3000 Series"^]. * Per ogni switch, le otto viti 10-32 o 12-24 e i dadi a clip per montare le staffe e le guide scorrevoli sui montanti anteriori e posteriori del cabinet. * Cisco standard rail kit per installare lo switch in un NetApp cabinet.
...
. Installare le staffe di montaggio su rack sullo chassis dello switch di storage Nexus 3232C.
+
...
+
I due switch 3232C saranno sempre montati nei 2U superiori dell'armadio RU41 e RU42.
...
====
. Collega la porta di gestione di ogni switch 3232C a uno degli switch di gestione (se ordinati) oppure collegali direttamente alla tua rete di gestione.
+
