...
[role="lead"]
Seguire questa procedura per installare RCF dopo aver configurato lo switch Nexus 3232C per la prima volta. È inoltre possibile utilizzare questa procedura per aggiornare la versione di RCF.
...
----
====
. Se non si dispone già di un backup corrente dello switch, è possibile salvare la configurazione corrente dello switch copiando l'output del seguente comando in un file di log:
+
`show running-config`
...
----
. Reboot switch cs1 (Riavvia switch cs1). Questa operazione consente ai file LIF del cluster di ripristinare le porte home. È possibile ignorare gli eventi di "interruzione delle porte del cluster" riportati sui nodi durante il riavvio dello switch.
+
.Mostra esempio
[%collapsible]
====
