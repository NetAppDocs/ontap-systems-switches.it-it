...
summary: 'Aggiorna il file di configurazione di riferimento (RCF) sugli switch di cluster Cisco Nexus 3232C quando è disponibile una versione più recente.' 
---
= Aggiorna l'RCF sugli switch di cluster Cisco Nexus 3232C
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Aggiorna il file di configurazione (RCF) sugli switch di cluster Cisco Nexus 3232C quando è disponibile una versione più recente dell'RCF.
...
[NOTE]
====
Potresti osservare il seguente output sulla console dello switch cs1 a seconda della versione RCF precedentemente caricata sullo switch 2020 Nov 17 16:07:18 cs1 %$ VDC-1 %$ %STP-2-UNBLOCK_CONSIST_PORT: Sblocco della porta port-channel1 sulla VLAN0092. Coerenza della porta ripristinata. 2020 Nov 17 16:07:23 cs1 %$ VDC-1 %$ %STP-2-BLOCK_PVID_PEER: Blocco di port-channel1 sulla VLAN0001. VLAN peer incoerente. 2020 Nov 17 16:07:23 cs1 %$ VDC-1 %$ %STP-2-BLOCK_PVID_LOCAL: Blocco di port-channel1 sulla VLAN0092. VLAN locale incoerente.
...
----
====
. Ripeti i passaggi da 4 a 19 sullo switch cs1.
...
--
Puoi usare il comando  `network interface check cluster-connectivity` per avviare un controllo di accessibilità della connettività del cluster e poi visualizzare i dettagli: 
...
--
Per tutte le release di ONTAP, puoi anche usare il comando 
`cluster ping-cluster -node <name>` per verificare la connettività:
