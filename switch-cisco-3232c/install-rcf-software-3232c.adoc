...
summary: 'Installa il file di configurazione di riferimento (RCF) sugli switch cluster Cisco Nexus 3232C dopo la configurazione iniziale.' 
---
= Installa l'RCF sugli switch di cluster Cisco Nexus 3232C
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Installa il file di configurazione di riferimento (RCF) sugli switch cluster Cisco Nexus 3232C dopo la configurazione iniziale. L'RCF definisce le assegnazioni delle porte e le impostazioni di rete necessarie per il funzionamento del cluster.
...
Durante questa procedura non è necessario alcun collegamento inter-switch (ISL) operativo. Questa scelta è intenzionale, perché le modifiche alla versione di RCF possono influire temporaneamente sulla connettività ISL. Per consentire operazioni di cluster senza interruzioni, la seguente procedura migra tutti i LIF del cluster allo switch partner operativo mentre esegui i passaggi sullo switch di destinazione.
...
. Accedi allo switch cs2 utilizzando SSH o utilizzando una console seriale.
...
== Passaggio 2: Verifica i collegamenti dello switch
...
====
+
Se alcuni LIFS del cluster non sono tornati alle loro porte di origine, ripristinali manualmente:
...
System Manager offre un flusso di lavoro semplice e intuitivo per la configurazione del cluster, che include l'assegnazione di un indirizzo IP di gestione del nodo, l'inizializzazione del cluster, la creazione di un tier locale, la configurazione dei protocolli e il provisioning dello storage iniziale.

Consulta  https://docs.netapp.com/us-en/ontap/task_configure_ontap.html["Configurare ONTAP su un nuovo cluster con System Manager"]le istruzioni di configurazione.
