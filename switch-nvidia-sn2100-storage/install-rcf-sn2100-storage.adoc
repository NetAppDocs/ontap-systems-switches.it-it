...
summary: 'Installa o aggiorna lo script del file di configurazione di riferimento (RCF) sugli switch di storage NVIDIA SN2100.' 
---
= Installa o aggiorna lo script RCF sugli switch di storage NVIDIA SN2100
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Installa o aggiorna lo script del file di configurazione di riferimento (RCF) sugli switch di storage NVIDIA SN2100 per configurare le assegnazioni delle porte e le impostazioni di rete.
...
NOTE: La seguente procedura di esempio mostra come scaricare e applicare lo script RCF per gli switch Cluster.


NOTE: L'output del comando di esempio utilizza l'indirizzo IP di gestione dello switch 10.233.204.71, la netmask 255.255.254.0 e il gateway predefinito 10.233.204.1.
...
----
====
. Copia lo script RCF Python sullo switch:
+
[listing, subs="+quotes"]
----
admin@sw1:mgmt:~$ *pwd*
...
----
+
Lo script RCF completa i passaggi elencati sopra.
...
. Riapplicare eventuali personalizzazioni precedenti alla configurazione dello switch.  Fare riferimento alink:cabling-considerations-sn2100-storage.html["Esaminare le considerazioni sul cablaggio e sulla configurazione"] per i dettagli di eventuali ulteriori modifiche richieste.
...
.Cosa succederà ora?
Dopo aver installato o aggiornato RCF, puoilink:setup-install-cshm-file.html["installare il file CSHM"] .
