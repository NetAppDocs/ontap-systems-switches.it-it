...
summary: Se si dispone di un ambiente cluster switchless a due nodi esistente, è possibile migrare a un ambiente cluster switchato a due nodi utilizzando gli switch di rete cluster CN1610 che consentono di scalare oltre due nodi. 
---
= Migrazione da un ambiente cluster switchless a un ambiente cluster NetApp CN1610 commutato
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Se si dispone di un ambiente cluster switchless a due nodi esistente, è possibile migrare a un ambiente cluster switchato a due nodi utilizzando gli switch di rete cluster CN1610 che consentono di scalare oltre due nodi.
...
* Tutte le interfacce logiche del cluster (LIF) sono nel `up` Stato e nei loro porti di origine.


Per la configurazione dello switch cluster CN1610:

* L'infrastruttura dello switch cluster CN1610 è completamente funzionante su entrambi gli switch.
...
* Le connessioni da nodo a nodo e da switch a switch CN1610 utilizzano cavi twinax o in fibra.
+
...
+
Qualsiasi precedente personalizzazione del sito, come SMTP, SNMP e SSH, deve essere copiata sui nuovi switch.
...
* Le porte cluster utilizzate in questa procedura sono e1a ed e2a.
+
...
[%collapsible]
====
L'esempio seguente mostra che le porte da 1 a 12 rivolte al nodo sono disabilitate sullo switch cs1:
...
(cs1)(Config)# exit
----
L'esempio seguente mostra che le porte da 1 a 12 rivolte al nodo sono disabilitate sullo switch cs2:
...
----
====
. Verificare che l'ISL e le porte fisiche sull'ISL tra i due switch cluster CN1610 cs1 e cs2 siano `up` :
+
`show port-channel`
...
0/16   actor/long    10G Full  True
       partner/long
----
L'esempio seguente mostra che le porte ISL sono `up` su switch cs2:
...
----
====
. Verificare che ogni porta del cluster sia connessa alla porta corrispondente sul nodo del cluster partner:
+
...
[%collapsible]
====
L'esempio seguente mostra che le porte del cluster e1a ed e2a sono collegate alla stessa porta sul nodo partner del cluster:
...
----
====
. Verificare che tutti i LIF del cluster siano `up` e operativo:
+
`network interface show -vserver Cluster`

+
Ogni cluster LIF dovrebbe visualizzare `true` nella colonna "`È casa`".
...
--
====
. [[step10]] Migrare clus1 sulla porta e2a sulla console di ciascun nodo:
+
...
[%collapsible]
====
L'esempio seguente mostra il processo di migrazione di clus1 alla porta e2a su node1 e node2:
...
. Verificare che la migrazione sia avvenuta:
+
`network interface show -vserver Cluster`
...
[%collapsible]
====
L'esempio seguente verifica che clus1 sia migrato sulla porta e2a su node1 e node2:
...
----
====
. Chiudere la porta e1a del cluster su entrambi i nodi:
+
`network port modify`
...
[%collapsible]
====
L'esempio seguente mostra come chiudere la porta e1a su node1 e node2:
...
[%collapsible]
====
L'esempio seguente mostra che la porta e1a è `down` su nodo1 e nodo2:
...
----
====
. Scollegare il cavo dalla porta e1a del cluster sul nodo 1, quindi collegare e1a alla porta 1 sullo switch cs1 del cluster, utilizzando il cablaggio appropriato supportato dagli switch CN1610.
+
ILlink:https://hwu.netapp.com/Switch/Index["Hardware Universe"^] contiene maggiori informazioni sul cablaggio.

. Scollegare il cavo dalla porta e1a del cluster sul nodo 2, quindi collegare e1a alla porta 2 sullo switch cs1 del cluster, utilizzando il cablaggio appropriato supportato dagli switch CN1610.
. Abilitare tutte le porte rivolte verso il nodo sullo switch del cluster cs1.
+
.Mostra esempio
[%collapsible]
====
L'esempio seguente mostra che le porte da 1 a 12 sono abilitate sullo switch cs1:
...
[%collapsible]
====
L'esempio seguente mostra come abilitare la porta e1a su node1 e node2:
...
[%collapsible]
====
L'esempio seguente mostra che tutte le porte del cluster sono `up` su nodo1 e nodo2:
...
----
====
. Ripristina clus1 (che era stato precedentemente migrato) su e1a su entrambi i nodi:
+
...
[%collapsible]
====
L'esempio seguente mostra come ripristinare clus1 sulla porta e1a su node1 e node2:
...
. Verificare che tutti i LIF del cluster siano `up` , operativo e visualizzato come `true` nella colonna "È a casa":
+
`network interface show -vserver Cluster`
...
----
====
. Migrare clus2 sulla porta e1a sulla console di ciascun nodo:
+
`network interface migrate`
...
[%collapsible]
====
L'esempio seguente mostra il processo di migrazione di clus2 alla porta e1a su node1 e node2:
...
[%collapsible]
====
L'esempio seguente verifica che clus2 sia migrato sulla porta e1a su node1 e node2:
...
[%collapsible]
====
L'esempio seguente mostra come chiudere la porta e2a su node1 e node2:
...
[%collapsible]
====
L'esempio seguente mostra che la porta e2a è `down` su nodo1 e nodo2:
...
----
====
. Scollegare il cavo dalla porta e2a del cluster sul nodo 1, quindi collegare e2a alla porta 1 sullo switch cs2 del cluster, utilizzando il cablaggio appropriato supportato dagli switch CN1610.
. Scollegare il cavo dalla porta e2a del cluster sul nodo 2, quindi collegare e2a alla porta 2 sullo switch cs2 del cluster, utilizzando il cablaggio appropriato supportato dagli switch CN1610.
...
[%collapsible]
====
L'esempio seguente mostra come abilitare la porta e2a su node1 e node2:
...
[%collapsible]
====
L'esempio seguente mostra come ripristinare clus2 sulla porta e2a su node1 e node2:
...
. Verificare che tutte le interfacce siano visualizzate `true` nella colonna "È a casa":
+
`network interface show -vserver Cluster`
...
--
====
. [[step3]] Verificare che entrambi i nodi abbiano due connessioni a ciascun switch:
+
`show isdp neighbors`
...
. Disabilitare le impostazioni di configurazione senza switch a due nodi su entrambi i nodi utilizzando il comando con privilegi avanzati:
+
...
[%collapsible]
====
L'esempio seguente mostra come disabilitare le impostazioni di configurazione senza switch:
...
NOTE: Per la versione 9.2 e successive, attendere fino a `Enable Switchless Cluster` è impostato su falso.  Questa operazione può richiedere fino a tre minuti.
...
NOTE: Per la versione 8.3 e successive, utilizzare il seguente comando: `network interface modify -vserver Cluster -lif * -auto-revert true` per abilitare il ripristino automatico su tutti i nodi del cluster.
