...
summary: 'Installa o aggiorna lo script del file di configurazione di riferimento (RCF) sugli switch di cluster NVIDIA SN2100.' 
---
= Installa o aggiorna lo script RCF sugli switch di cluster NVIDIA SN2100
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Installa o aggiorna lo script del file di configurazione di riferimento (RCF) sugli switch di cluster NVIDIA SN2100 per configurare le assegnazioni delle porte e le impostazioni di rete.

.Prima di iniziare
Prima di installare o aggiornare lo script RCF, assicurarsi che sullo switch siano disponibili i seguenti elementi:

* Cumulus Linux è installato. Consultare la link:https://hwu.netapp.com/Switch/Index["Hardware Universe"^] per le versioni supportate.
* Indirizzo IP, maschera di sottorete e gateway predefinito definiti tramite DHCP o configurati manualmente.



NOTE: È necessario specificare un utente nel RCF (oltre all'utente amministratore) da utilizzare specificamente per la raccolta dei log.

.Configurazioni del cliente
Sono disponibili le seguenti categorie di configurazione di riferimento:

[cols="1,2"]
|===


| Grappolo | Sulle porte configurate per breakout 4x10GbE, una porta è configurata per breakout 4x25GbE e le altre porte sono configurate per 40/100GbE. Supporta il traffico cluster/HA condiviso sulle porte per i nodi che utilizzano porte cluster/HA condivise. Fare riferimento alla tabella della piattaforma nell'articolo della Knowledge Base link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/What_AFF_ASA_and_FAS_platforms_use_shared_Cluster_and_HA_Ethernet_ports["Quali piattaforme AFF, ASA e FAS utilizzano porte Ethernet Cluster e HA condivise?"^]. Tutte le porte possono essere utilizzate anche come porte cluster dedicate. 
...
|===
.Versioni attuali dello script RCF
Sono disponibili due script RCF per le applicazioni Cluster e Storage.  Scarica RCF da https://mysupport.netapp.com/site/info/nvidia-cluster-switch["Download del software NVIDIA SN2100"^] pagina.  La procedura è la stessa per entrambi.

* Cluster: *MSN2100-RCF-v1._x_-Cluster-HA-Breakout-LLDP*
* Archiviazione: *MSN2100-RCF-v1._x_-Archiviazione*


.Informazioni sugli esempi
La seguente procedura di esempio mostra come scaricare e applicare lo script RCF per gli switch Cluster.

L'output del comando di esempio utilizza l'indirizzo IP di gestione dello switch 10.233.204.71, la netmask 255.255.254.0 e il gateway predefinito 10.233.204.1.
...
====
. Visualizza le interfacce disponibili sullo switch SN2100:
+
[listing, subs="+quotes"]
----
...
----
. Copiare lo script RCF Python sullo switch.
+
[listing, subs="+quotes"]
----
...
NOTE: Mentre `scp` viene utilizzato nell'esempio, puoi usare il tuo metodo preferito per il trasferimento dei file, ad esempio SFTP, HTTPS o FTP.
...
----
+
Lo script RCF completa i passaggi elencati nell'esempio precedente.

+

NOTE: Nel passaggio 3 *Aggiornamento del file MOTD* sopra, il comando `cat /etc/motd` è in esecuzione.  Ciò consente di verificare il nome del file RCF, la versione RCF, le porte da utilizzare e altre informazioni importanti nel banner RCF.

+

NOTE: Per qualsiasi problema relativo allo script python RCF che non può essere corretto, contattare https://mysupport.netapp.com/["Supporto NetApp"^] per assistenza.

. Riapplicare eventuali personalizzazioni precedenti alla configurazione dello switch.  Fare riferimento alink:cabling-considerations-sn2100-cluster.html["Esaminare le considerazioni sul cablaggio e sulla configurazione"] per i dettagli di eventuali ulteriori modifiche richieste.
...
5                 5  DWRR 43%
----
. Verificare le informazioni per il ricetrasmettitore nell'interfaccia:
+
[listing, subs="+quotes"]
----
...
e3b       Cluster      Cluster          up   9000  auto/10000 healthy  false
----
.. Verificare lo stato di integrità dello switch dal cluster (potrebbe non essere visualizzato lo switch sw2, poiché i LIF non sono posizionati su e0d).
+
[listing, subs="+quotes"]
----
...
NOTE: Nel passaggio 3 *Aggiornamento del file MOTD* sopra, il comando `cat /etc/issue.net` è in esecuzione.  Ciò consente di verificare il nome del file RCF, la versione RCF, le porte da utilizzare e altre informazioni importanti nel banner RCF.
...
NOTE: Le eccezioni elencate non influiscono sulle prestazioni e possono essere ignorate senza problemi.
...
NOTE: Sebbene `scp` viene utilizzato nell'esempio, puoi usare il tuo metodo preferito per il trasferimento dei file, ad esempio SFTP, HTTPS o FTP.
...
NOTE: Nel passaggio 3 *Aggiornamento del file MOTD* sopra riportato, viene eseguito il comando `cat /etc/issue.net.  Ciò consente di verificare il nome del file RCF, la versione RCF, le porte da utilizzare e altre informazioni importanti nel banner RCF.
...
====
.Cosa succederà ora?
Dopo aver installato l'RCF, puoi link:setup-install-cshm-file.html["installare il file CSHM"].
