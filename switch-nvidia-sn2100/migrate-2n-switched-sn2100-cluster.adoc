...
summary: Migra un cluster senza switch a due nodi esistente a un ambiente cluster con switch utilizzando gli switch cluster NVIDIA SN2100. 
---
...
[role="lead"]
Esegui la migrazione di un cluster senza switch a due nodi esistente a un ambiente cluster con switch utilizzando gli switch cluster NVIDIA SN2100. Questa migrazione senza interruzioni ti permette di scalare il cluster oltre due nodi.
...
IMPORTANT: Consultare link:cabling-considerations-sn2100-cluster.html["Esaminare le considerazioni sul cablaggio e sulla configurazione"] per avvertenze e ulteriori dettagli. Il link:https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe - Interruttori"^] contiene anche maggiori informazioni sul cablaggio.
...
--
====
. [[step8]]Riporta il livello di privilegio su amministratore:
+
`set -privilege admin`
