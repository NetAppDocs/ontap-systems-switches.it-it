...
summary: 'Aggiorna lo script del file di configurazione di riferimento (RCF) sugli switch di cluster NVIDIA SN2100 quando è disponibile una versione più recente.' 
---
= Aggiorna lo script RCF sugli switch di cluster NVIDIA SN2100
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Aggiorna lo script del file di configurazione di riferimento (RCF) sugli switch di cluster NVIDIA SN2100 quando è disponibile una versione più recente.

.Prima di iniziare
Prima di aggiornare lo script RCF, assicurarsi che sullo switch siano disponibili i seguenti elementi:
...
NOTE: Sebbene `scp` viene utilizzato nell'esempio, puoi usare il tuo metodo preferito per il trasferimento dei file.
...
NOTE: Nel passaggio 3 *Aggiornamento del file MOTD* sopra, il comando `cat /etc/issue` è in esecuzione.  Ciò consente di verificare il nome del file RCF, la versione RCF, le porte da utilizzare e altre informazioni importanti nel banner RCF.
...
NOTE: Mentre `scp` viene utilizzato nell'esempio, puoi usare il tuo metodo preferito per il trasferimento dei file.
