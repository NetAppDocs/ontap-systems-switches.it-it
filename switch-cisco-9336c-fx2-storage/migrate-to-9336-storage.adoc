...
. Sullo switch cluster CS2, chiudere le porte collegate alle porte del cluster dei nodi per eseguire il failover delle LIF del cluster:
+
[listing, subs="+quotes"]
----
...
----
. Nello switch cluster CS1, chiudere le porte collegate alle porte del cluster dei nodi per eseguire il failover delle LIF del cluster. Nell'esempio seguente viene utilizzato l'output dell'esempio di interfaccia del passo 7.
+
[listing, subs="+quotes"]
----
