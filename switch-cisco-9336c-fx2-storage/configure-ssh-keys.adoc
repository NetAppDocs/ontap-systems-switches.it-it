...
summary: Verifica che SSH e le relative chiavi siano abilitati sugli switch di storage Cisco Nexus 9336C-FX2 per supportare CSHM e la raccolta dei log. 
---
= Verifica la configurazione SSH per gli switch di storage Cisco Nexus 9336C-FX2
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Verifica che SSH e le chiavi SSH siano abilitati sugli switch di storage Cisco Nexus 9336C-FX2 e 9336C-FX2-T per supportare le funzionalità di health monitor degli switch Ethernet (CSHM) e di raccolta dei log.
...
IMPORTANT: Quando si abilita FIPS, è necessario modificare il bitcount a 256 sullo switch utilizzando il comando `ssh key ecdsa 256 force`. Consultare link:https://docs.netapp.com/us-en/ontap/networking/configure_network_security_using_federal_information_processing_standards_@fips@.html#enable-fips["Configurare la sicurezza di rete utilizzando FIPS"^] per ulteriori dettagli.
