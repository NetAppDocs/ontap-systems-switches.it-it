...
. [[step5]]verifica che tutte le LIF del cluster siano in grado di comunicare:
+
...
. [[step13]]verifica che tutte le LIF del cluster siano in grado di comunicare:
+
...
. [[step15]]attivare la funzione di raccolta dei log dello switch Ethernet per la raccolta dei file di log relativi allo switch, utilizzando i seguenti comandi:
+
...
. [[step16]]spostare le porte di storage dal vecchio switch sh2 al nuovo switch news2.
...
. [[step19]]verificare che gli shelf siano cablati correttamente:
+
...
. [[step20]]rimuovere il vecchio switch sh2.
