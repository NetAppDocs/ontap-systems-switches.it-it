...
.. Per NX-OS 10.2 e versioni successive utilizzare `show diff running-config` comando da confrontare con il file RCF salvato nel bootflash. In caso contrario, utilizzare uno strumento di confronto/differenza di terze parti.
...
CAUTION: Assicuratevi di leggere attentamente le sezioni *Note di installazione*, *Note importanti* e *banner* del vostro RCF. Per garantire la corretta configurazione e il corretto funzionamento dello switch, è necessario leggere e seguire queste istruzioni.
...
. Verificare lo stato delle porte sul cluster.
+
.. *Porte cluster*
+
