...
summary: Aggiorna NX-OS sugli switch Cisco Nexus 9364D-GX2A per supportare nuovo hardware o una nuova versione di ONTAP sui sistemi AFX 1K o AFX 2K. 
---
= Aggiorna NX-OS sullo switch Cisco Nexus 9364D-GX2A
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Aggiorna NX-OS sugli switch Cisco Nexus 9364D-GX2A per supportare nuovo hardware o una nuova versione di ONTAP sui sistemi AFX 1K o AFX 2K.
...
* Se necessario, puoi consultare la link:https://www.cisco.com/c/dam/en/us/td/docs/dcn/tools/nexus-9k3k-issu-matrix/index.html["Aggiornamento Cisco Nexus 9000 e 3000 e matrice ISSU"^], che fornisce informazioni sull'In-Service Software Upgrade (ISSU) per gli switch Cisco Nexus 9000 Series in base alle tue versioni corrente e di destinazione.
+
...
Installi NX-OS sul primo switch (cs1) installando e verificando le nuove versioni di NX-OS ed EPLD, verificando lo stato di salute di tutte le porte del cluster e poi ripetendo questi passaggi sul secondo switch (cs2).

. Usa l'interfaccia a riga di comando dello switch per copiare le immagini NX-OS ed EPLD su uno switch.
+
...
----
====
. Utilizzando l'interfaccia a riga di comando dello switch, installa l'immagine NX-OS.
+
...
----
====
. Tramite l'interfaccia a riga di comando dello switch, aggiorna l'immagine EPLD utilizzando i seguenti comandi:
+
...
----
====
. Dopo il riavvio dello switch, usa la CLI dello switch per accedere e verificare che la nuova versione di EPLD sia stata caricata correttamente.
+
`show version module 1 epld`
...
----
====
. Ripeti questi passaggi per installare le immagini NX-OS e EPLD sullo switch cs2.
...
.Cosa succederà ora?
Dopo che hai aggiornato NX-OS sugli switch, aggiorni l'RCF a partire da link:configure-install-upgrade-rcf-overview-9364d.html["Installa o aggiorna la panoramica RCF"].
