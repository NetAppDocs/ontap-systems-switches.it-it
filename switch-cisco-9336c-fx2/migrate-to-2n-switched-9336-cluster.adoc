...
summary: Migra un cluster senza switch a due nodi esistente in un ambiente cluster con switch utilizzando gli switch cluster Cisco Nexus 9336C-FX2 e 9336C-FX2-T. 
---
= Esegui la migrazione a un cluster a due nodi con switch di cluster Cisco Nexus 9336C-FX2
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Esegui la migrazione di un cluster senza switch a un ambiente cluster con switch utilizzando gli switch cluster Cisco Nexus 9336C-FX2 e 9336C-FX2-T. Questa migrazione senza interruzioni ti permette di scalare il cluster oltre due nodi.
...
Consultare la link:https://hwu.netapp.com["Hardware Universe"^] per informazioni sulle porte del cluster per le proprie piattaforme. Consultare link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["Quali informazioni aggiuntive mi servono per installare la mia attrezzatura che non è presente in HWU?"^] per ulteriori informazioni sui requisiti di installazione dello switch.
...
[%collapsible]
====
Il seguente esempio mostra che tutti i LIF sono attivi su node1 e node2:
