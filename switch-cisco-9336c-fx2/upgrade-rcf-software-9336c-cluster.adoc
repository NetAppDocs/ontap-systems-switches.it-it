...
summary: 'Aggiorna il file di configurazione (RCF) sugli switch di cluster Cisco Nexus 9336C-FX2 e 9336C-FX2-T quando è disponibile una versione più recente.' 
---
= Aggiorna l'RCF sugli switch di cluster Cisco Nexus 9336C-FX2 e 9336C-FX2-T
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Aggiorna il file di configurazione di riferimento (RCF) sugli switch di cluster Cisco Nexus 9336C-FX2 e 9336C-FX2-T quando è disponibile una versione più recente dell'RCF.
...
.. A partire da NX-OS 10.2, utilizzare `show diff running-config` comando per confrontare con il file RCF salvato nel bootflash. In caso contrario, utilizzare uno strumento di confronto/differenza di terze parti.
...
. Emettere il comando write erase per cancellare la configurazione salvata corrente:
+
`cs1# *write erase*`
...
. Copia il file RCF nella memoria bootflash dello switch cs1 utilizzando uno dei seguenti protocolli di trasferimento: FTP, SFTP o SCP.
+
Per ulteriori informazioni sui comandi Cisco, fare riferimento alla guida appropriata nella link:https://www.cisco.com/c/en/us/support/switches/nexus-9336c-fx2-switch/model.html#CommandReferences["Riferimento ai comandi NX-OS della serie Cisco Nexus 9000"^].

+
Questo esempio mostra SFTP utilizzato per copiare un RCF nel bootflash sullo switch cs1:
...
. Riapplicare eventuali personalizzazioni precedenti alla configurazione dello switch.  Fare riferimento alink:cabling-considerations-9336c-fx2.html["Esaminare le considerazioni sul cablaggio e sulla configurazione"] per i dettagli di eventuali ulteriori modifiche richieste.
. Dopo aver verificato che le versioni RCF, le aggiunte personalizzate e le impostazioni degli switch siano corrette, copiare il file running-config nel file startup-config.
+
Per ulteriori informazioni sui comandi Cisco, fare riferimento alla guida appropriata nella link:https://www.cisco.com/c/en/us/support/switches/nexus-9336c-fx2-switch/model.html#CommandReferences["Riferimento ai comandi NX-OS della serie Cisco Nexus 9000"^].
...
----
====
. Ripetere i passaggi da 1 a 19 sullo switch cs2.
. Abilita il ripristino automatico sui LIF del cluster.
+
`cluster1::*> *network interface modify -vserver Cluster -lif * -auto-revert True*`

. Eseguire un riavvio di switch cs2.
+
`cs2# *reload*`
...
CAUTION: Se lo switch 9336C-FX2 non si avvia dopo l'aggiornamento RCF, consultare l'NetApp articolo della Knowledge Base link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/Cisco_9336C-FX2_Switch_Fails_to_Boot_After_RCF_Install_%E2%80%93_Boot_Variable_Cleared["Cisco 9336C-FX2 Switch non si avvia dopo l'installazione di RCF – Variabile di boot cancellata"^] per i passaggi successivi.
