...
---
= Sostituisci gli switch cluster Cisco Nexus 3132Q-V con connessioni switchless
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[NOTE]
====
NetApp consiglia di aggiornare la versione ONTAP prima di procedere con il passaggio da un cluster switchless a uno switch per gli switch Cisco Nexus 3132Q-V.

Per maggiori dettagli vedere quanto segue:
...
* link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/Node_panic_after_migration_from_switched_to_switchless_cluster["Nodo in panico dopo la migrazione da un cluster commutato a uno senza commutazione"^]
