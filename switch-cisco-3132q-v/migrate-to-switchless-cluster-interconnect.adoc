...
summary: Sostituisci gli switch cluster Cisco Nexus 3132Q-V con connessioni senza switch per ONTAP 9.3 e versioni successive. 
---
= Sostituisci gli switch cluster Cisco Nexus 3132Q-V con connessioni senza switch
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
In ONTAP 9.3 e versioni successive, puoi migrare da un cluster con interconnessioni tramite switch a uno in cui due nodi sono collegati direttamente, sostituendo gli switch cluster Cisco Nexus 3132Q-V con connessioni dirette.

[NOTE]
====
NetApp ti consiglia di aggiornare la versione di ONTAP prima di procedere con l'operazione di passaggio da cluster con switch a cluster senza switch per gli switch Cisco Nexus 3132Q-V.

Per maggiori dettagli, consulta quanto segue:

* link:https://kb.netapp.com/Support_Bulletins/Customer_Bulletins/SU540["SU540: Gli errori della scheda di rete Chelsio T6 causano l'arresto del sistema durante l'aggiornamento da switch di rete 40G a switch di rete 100G"^]
* link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/Node_panic_after_migration_from_switched_to_switchless_cluster["Errore del nodo dopo la migrazione da cluster con switch a cluster senza switch"^]
