...
summary: 'Aggiorna il file di configurazione di riferimento (RCF) sugli switch di cluster Cisco Nexus 3132Q-V quando è disponibile una versione più recente.' 
---
= Aggiorna l'RCF sugli switch di cluster Cisco Nexus 3132Q-V
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Aggiorna il file di configurazione (RCF) sugli switch di cluster Cisco Nexus 3132Q-V quando è disponibile una versione più recente dell'RCF.
...
NOTE: Per ONTAP 9.8 e versioni successive, usa il comando  `system switch ethernet show -is-monitoring-enabled-operational true`.
...
----
+
Assicurati che il ripristino automatico sia disabilitato dopo aver eseguito questo comando.
...
. Verifica che le porte del cluster siano passate alle porte ospitate sullo switch del cluster cs1. Questo potrebbe richiedere alcuni secondi.
+
`network interface show -vserver Cluster`
...
. Annota eventuali aggiunte personalizzate tra la running-config attuale e il file RCF in uso.
+
[NOTE]
====
...
NOTE: Quando si aggiorna o si applica un nuovo RCF, è necessario cancellare le impostazioni dello switch ed eseguire la configurazione di base.
...
+
Per ulteriori informazioni sui comandi Cisco, fare riferimento alla guida appropriata in  https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Riferimento ai comandi di Cisco Nexus 3000 Series NX-OS"^].
...
NOTE: Per le istruzioni su come attivare le porte 10GbE dopo un aggiornamento dell'RCF, consultare l'articolo della Knowledge Base link:https://kb.netapp.com/onprem%2FSwitches%2FCisco%2F10GbE_ports_on_Cisco_3132Q_cluster_switch_do_not_come_online["Le porte 10GbE su uno switch di cluster Cisco 3132Q non si connettono"^].



. Dopo che hai verificato che le versioni RCF e le impostazioni dello switch sono corrette, copia il file  `running-config` nel file  `startup-config`.
+
Per ulteriori informazioni sui comandi Cisco, fare riferimento alla guida appropriata nella link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Riferimento ai comandi di Cisco Nexus 3000 Series NX-OS"^]. .Mostra esempio
...
----
====
. Riavvia lo switch cs2. Puoi ignorare sia gli eventi "cluster ports down" segnalati sui nodi durante il riavvio dello switch sia l'output di errore `% Invalid command at '^' marker`.
+
[listing, subs="+quotes"]
----
cs2# *reload*
This command will reboot the system. (y/n)?  [n] *y*
----
. Riapplica alla configurazione dello switch tutte le personalizzazioni precedenti. Consulta link:cabling-considerations-3132q-v.html["Esaminare le considerazioni sul cablaggio e sulla configurazione"] per i dettagli relativi a eventuali ulteriori modifiche necessarie.
...
----
====
. Verifica che le LIF del cluster siano tornate alle loro porte principali:
+
`network interface show -vserver Cluster`
