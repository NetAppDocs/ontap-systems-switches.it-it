...
summary: 'È possibile installare l"RCF dopo aver configurato per la prima volta lo switch Nexus 3132Q-V. Puoi usare questa procedura anche per aggiornare la tua versione RCF.' 
---
...
* link:https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Switch Ethernet Cisco"^]. Consultare la tabella di compatibilità degli switch per le versioni ONTAP e RCF supportate.  Si noti che possono esserci dipendenze tra la sintassi dei comandi nella RCF e quella presente nelle versioni di NX-OS.
...
----
====
. Esaminare l'output del banner dal `show banner motd` comando.  Per garantire la corretta configurazione e il corretto funzionamento dello switch, è necessario leggere e seguire le istruzioni riportate nella sezione *Note importanti*.
+
.Mostra esempio
[%collapsible]
====
...
----
====
. Applicare lo stesso RCF e salvare la configurazione in esecuzione per una seconda volta.  Ciò è necessario poiché l'RCF utilizza QoS e richiede la riconfigurazione del TCAM, che comporta il caricamento dell'RCF due volte, con lo switch riavviato nel frattempo.
+
.Mostra esempio
[%collapsible]
====
